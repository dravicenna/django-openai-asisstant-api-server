...
def setup_routes(app, client, tool_data, assistant_id):
    ...
    async def on_message(message):
        ...
        logging.info(f'Received message from Discord user ID: {discord_user_id} for OpenAI thread ID: {thread_id}')

        if not thread_id:
            logging.error('Error: Missing OpenAI thread_id')
            return jsonify({'error': 'Missing OpenAI thread_id'}), 400

        logging.info(f'Received message: {user_input} for OpenAI thread ID: {thread_id}')
